...
//------------------------Parameter----------------------
`define TV_IN_gmem3 "../tv/cdatafile/c.sobel_resize_accel.autotvin_gmem3.dat"
`define TV_OUT_gmem3 "../tv/rtldatafile/rtl.sobel_resize_accel.autotvout_gmem3.dat"
...
parameter gmem3_C_DATA_BITWIDTH = 32'd 8;
parameter gmem3_transaction_depth = 32'd 4096;
parameter gmem3_mem_depth = 32'd 1024;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem3_mem_depth) begin
                $display ("/home/sxy/Desktop/FPGA_Camp24-main/Lab1_sobel/prj/sobel_resize/sobel_resize_xf/sol2/sim/verilog/AESL_axi_master_gmem3.v: Write request address %d exceed AXI master gmem3 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem3_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem3_mem_depth) begin
                $display ("/home/sxy/Desktop/FPGA_Camp24-main/Lab1_sobel/prj/sobel_resize/sobel_resize_xf/sol2/sim/verilog/AESL_axi_master_gmem3.v: Read request address %d exceed AXI master gmem3 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem3_mem_depth); 
                $finish;
            end
...
initial begin : read_file_process 
  ...
  while (trans_depth != 64'h5a5aa5a50f0ff0f0)  begin
      ...
      for(i = 0; i < gmem3_transaction_depth; i = i + 1) begin 
          ...
          if (i%4 == 3) begin
              case(mem_page)
                  0 : gmem3_mem_0[i/4] = mem_tmp;
                  1 : gmem3_mem_1[i/4] = mem_tmp;
                  2 : gmem3_mem_2[i/4] = mem_tmp;
                  3 : gmem3_mem_3[i/4] = mem_tmp;
...
initial begin : write_file_proc 
  ...
  while(1) begin 
      ...
      bin_data = gmem3_transaction_depth;
      write_binary(fp,bin_data,64);
      for(i = 0; i < gmem3_mem_depth; i = i + 1) begin 
          case(mem_page)
              0 : tmp_data = gmem3_mem_0[i];
              1 : tmp_data = gmem3_mem_1[i];
              2 : tmp_data = gmem3_mem_2[i];
              3 : tmp_data = gmem3_mem_3[i];
